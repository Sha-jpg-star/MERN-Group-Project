...
const Sidebar = () => {
  return (
    <Drawer variant="permanent" sx={{ width: 250, "& .MuiDrawer-paper": { width: 250, backgroundColor: "purple", color: "#ffff", alignItems: "center" } }}>
      {/* Logo and Title */}
      <img src="/landingPageIcon2.png" alt="Logo" style={{ height: "40px", padding: "10px", width: "40px", marginTop: "5px", borderRadius: "50%" }} />
      <Typography variant="h6" sx={{ flexGrow: 1, color: "yellow", textAlign: "center", fontWeight: "bold" }}>
        MediCare
      </Typography>
      ...
      <List sx={{width:"200px"}}>
        {[
          ...
          { text: 'Lab Reports', path: "/LabReports", icon: <DescriptionIcon /> },
          { text: 'Appointment', path: "/Appointment", icon: <EventAvailableIcon /> },
          { text: "Messages", path: "/Messages", icon: <MessageIcon /> },
          { text: "Settings", path: "/Settings", icon: <SettingsIcon /> },
          { text: "Logout", path: "/Logout", icon: <ExitToAppIcon /> },
        ].map((item, index) => (
          <ListItem button key={index} sx={{ backgroundColor: "rgb(155, 8, 155)" }}>
            {/* Link for Navigation */}
